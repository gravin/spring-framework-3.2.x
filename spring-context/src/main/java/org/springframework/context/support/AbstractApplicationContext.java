...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		/**
		 * 在 obtainFreshBeanFactory() 这一步会注册 用来解析 load-time-weaver 节点的  Bean
		 * @see LoadTimeWeaverBeanDefinitionParser#resolveId(Element, AbstractBeanDefinition, ParserContext)
		 * @see ContextNamespaceHandler#init()
		 * 如此，如果配置了 <context:load-time-weaver /> 此处便会执行
		 */
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		for (String postProcessorName : orderedPostProcessorNames) {
			 ...
			 * 例：
			 * invokeBeanFactoryPostProcessors 执行到这里时
			 * 因为 AspectJWeavingEnabler 继承了BeanFactoryPostProcessor 及 Ordered接口，
			 * 且在 {@link org.springframework.context.config.LoadTimeWeaverBeanDefinitionParser#doParse(Element, ParserContext, BeanDefinitionBuilder)} 下附带完成注册
			 * 所以 orderedPostProcessorNames 包含了 org.springframework.context.weaving.AspectJWeavingEnabler#0
			 *
			 * 于是此处方法会被调用: getBean("name": "org.springframework.context.weaving.AspectJWeavingEnabler#0","requiredType": "interface org.springframework.beans.factory.config.BeanFactoryPostProcessor")
			 * 则会调用到所有BeanPostProcessor接口的 postProcessBeforeInitialization 接口方法，
			 * 其中LoadTimeWeaverAwareProcessor 方法中作了 if (bean instanceof LoadTimeWeaverAware)的判断，则该BeanPostProcessor只对AspectJWeavingEnabler产生作用。
			 *
			 ...
			 * 此时将会获得名字为loadTimeWeaver，类型为DefaultContextLoadTimeWeaver的Bean(原因见下), 由于该类型实现了BeanClassLoaderAware，那则会在initializeBean步骤调用invokeAwareMethods, 从而调用 DefaultContextLoadTimeWeaver.setBeanClassLoader方法
			 ...
			 * 此处实由LoadTimeWeaverBeanDefinitionParser的父类AbstractBeanDefinitionParser完成注册
